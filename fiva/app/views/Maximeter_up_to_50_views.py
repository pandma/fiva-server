...
from ..models.Maximeter_up_to_50 import Maximeter_up_to_50
import json


class Maximeter_up_to_50Views(View):
    ...
    def get(self, request):
        max = list(Maximeter_up_to_50.objects.values())
        if len(max) > 0:
    ...
    def post(self, request):
        ...
        Maximeter_up_to_50.objects.create(
            owner=jd['owner'],
    ...
    def put(self, request, id):
        jd = json.loads(request.body)
        maxs = list(Maximeter_up_to_50.objects.filter(id=id).values())
        if len(maxs) > 0:
            max = Maximeter_up_to_50.objects.get(id=id)
